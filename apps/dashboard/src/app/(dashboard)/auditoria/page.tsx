    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Trilha de auditoria imutável com encadeamento SHA-256</p>
        </div>
        <button
          onClick={() =>
            api.get('/auditoria/verificar-cadeia').then((r) => {
              const { valida, total } = r.data
              alert(valida ? `✓ Cadeia íntegra — ${total} evento(s) verificados` : '✗ INTEGRIDADE COMPROMETIDA!')
            })
          }
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-5">
          ...
          <div className="space-y-2">
            {(pendentes as any[]).map((ev: any) => (
              <div key={ev.id} className="flex items-center justify-between bg-white rounded-lg px-4 py-3 shadow-sm">
                <div>
                  <p className="text-sm font-medium text-slate-900">
                    {ev.entidadeTipo} — <span className="font-mono text-xs">{ev.entidadeId.slice(0, 8)}...</span>
                  </p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Score: <span className="font-semibold text-yellow-700">{ev.score}</span>
                    {ev.cnpj && <> · CNPJ: <span className="font-mono">{ev.cnpj}</span></>}
                    {' · '}{new Date(ev.timestamp).toLocaleString('pt-BR')}
                  </p>
                </div>
                <button
                  onClick={() => { setAprovandoId(ev.id); aprovar.mutate(ev.id) }}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3 flex-wrap">
          <input
            ...
            value={filtroCnpj}
            onChange={(e) => { setFiltroCnpj(e.target.value); setPagina(1) }}
            className="border border-slate-300 rounded-lg px-3 py-1.5 text-sm w-48"
          />
          <select
            value={filtroEvento}
            onChange={(e) => { setFiltroEvento(e.target.value); setPagina(1) }}
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase">Entidade</th>
                ...
                {paginaAtual.map((ev: any) => (
                  <tr key={ev.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded font-mono ${EVENTO_COR[ev.evento] ?? 'bg-slate-100 text-slate-600'}`}>
                        {ev.evento}
                      </span>
                    </td>
            ...
            {totalPaginas > 1 && (
              <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-xs text-slate-400">
                  Página {pagina} de {totalPaginas} · mostrando {paginaAtual.length} de {filtrados.length}
                </span>
