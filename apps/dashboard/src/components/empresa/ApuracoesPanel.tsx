...
export function ApuracoesPanel({ empresaId, competencia }: Props) {
  ...
  const { data: apuracoes = [] } = useQuery({
    ...
    queryFn: () => api.get(`/fiscal/apuracoes/${empresaId}`, { params: { competencia } }).then((r) => r.data),
  })
  ...
  return (
    <div className="space-y-6">
      ...
      {apuracoes.length === 0 ? (
        <div className="text-center py-12 text-slate-500 text-sm">Nenhuma apuração encontrada para {competencia}</div>
      ) : (
              ...
              {ap.tipo === 'PGDAS' && ap.dados && (
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between text-slate-600">
                    ...
                    <span className="font-mono">R$ {Number(ap.dados.receitaBrutaTotal ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                  </div>
                  <div className="flex justify-between text-slate-600">
                  ...
                  <div className="flex justify-between font-semibold text-slate-900 pt-2 border-t border-slate-100">
                    <span>Valor DAS</span>
                    <span className="font-mono text-blue-600">R$ {Number(ap.dados.valorDAS ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
              )}
            </div>
