/**
 * Adapter — Prefeitura de Fortaleza (IBGE 2304400)
 * Portal: NFS-e Fortaleza — https://nfse.fortaleza.ce.gov.br
 */
...
const BASE_URL = 'https://nfse.fortaleza.ce.gov.br'
const TAG = 'prefeitura_fortaleza'
...
export class Prefeitura2304400Adapter extends BasePLaywrightAdapter implements PrefeituraAdapter {
  tipo = 'NFSE_EMITIDA'
  fonte = 'PREFEITURA_FORTALEZA'
  municipio = 'Fortaleza'
  ibge = '2304400'
      ...
      const session = await this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-form`,
            `Prefeitura Fortaleza: formulário de login não encontrado`
          )
        }
        ...
        if (await page.isVisible(SEL.MSG_ERRO_LOGIN)) {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-erro`,
            `Prefeitura Fortaleza: falha de autenticação — ${msg?.trim() ?? 'erro desconhecido'}`
          )
        }
        ...
        if (page.url().toLowerCase().includes('login')) {
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-redirect`,
            `Prefeitura Fortaleza: ainda na tela de login após submit`
          )
        }
      ...
      this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-xml-${doc.numero}`,
            `Prefeitura Fortaleza: falha download XML nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
      ...
      this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-pdf-${doc.numero}`,
            `Prefeitura Fortaleza: falha download PDF nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
  ...
  private async garantirSessao(page: Page, cnpj: string, ctx: string): Promise<void> {
    if (!this.session)
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-sem-sessao-${ctx}`,
        `Prefeitura Fortaleza: sessão não inicializada — chame authenticate() primeiro`
      )
    if (this.session!.expiresAt < nowBR())
      return this.captureAndThrow(
        ...
        `Prefeitura Fortaleza: sessão expirada — reautentique`
      )
  }
  ...
  private async preencherFiltro(
    ...
    try {
      ...
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-filtro-${ctx}`,
        `Prefeitura Fortaleza: formulário de consulta não encontrado (${ctx})`
      )
    }
