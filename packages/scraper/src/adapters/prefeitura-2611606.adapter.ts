/**
 * Adapter — Prefeitura de Recife (IBGE 2611606)
 * Portal: NFS-e Recife — https://nfse.recife.pe.gov.br
 */
...
const BASE_URL = 'https://nfse.recife.pe.gov.br'
const TAG = 'prefeitura_recife'
...
export class Prefeitura2611606Adapter extends BasePLaywrightAdapter implements PrefeituraAdapter {
  tipo = 'NFSE_EMITIDA'
  fonte = 'PREFEITURA_RECIFE'
  municipio = 'Recife'
  ibge = '2611606'
      ...
      const session = await this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-form`,
            `Prefeitura Recife: formulário de login não encontrado`
          )
        }
        ...
        if (await page.isVisible(SEL.MSG_ERRO_LOGIN)) {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-erro`,
            `Prefeitura Recife: falha de autenticação — ${msg?.trim() ?? 'erro desconhecido'}`
          )
        }
        ...
        if (page.url().toLowerCase().includes('login')) {
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-redirect`,
            `Prefeitura Recife: ainda na tela de login após submit`
          )
        }
      ...
      this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-xml-${doc.numero}`,
            `Prefeitura Recife: falha download XML nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
      ...
      this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-pdf-${doc.numero}`,
            `Prefeitura Recife: falha download PDF nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
  ...
  private async garantirSessao(page: Page, cnpj: string, ctx: string): Promise<void> {
    if (!this.session)
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-sem-sessao-${ctx}`,
        `Prefeitura Recife: sessão não inicializada — chame authenticate() primeiro`
      )
    if (this.session!.expiresAt < nowBR())
      return this.captureAndThrow(
        ...
        `Prefeitura Recife: sessão expirada — reautentique`
      )
  }
  ...
  private async preencherFiltro(
    ...
    try {
      ...
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-filtro-${ctx}`,
        `Prefeitura Recife: formulário de consulta não encontrado (${ctx})`
      )
    }
