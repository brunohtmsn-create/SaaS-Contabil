/**
 * Adapter — Prefeitura de Vila Velha (IBGE 3205200)
 * Portal: NFS-e Vila Velha — https://nfse.vilavelha.es.gov.br
 */
...
const BASE_URL = 'https://nfse.vilavelha.es.gov.br'
const TAG = 'prefeitura_vila_velha'
...
export class Prefeitura3205200Adapter extends BasePLaywrightAdapter implements PrefeituraAdapter {
  tipo = 'NFSE_EMITIDA'
  fonte = 'PREFEITURA_VILA_VELHA'
      ...
      const session = await this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-form`,
            `Prefeitura Vila Velha: formulário de login não encontrado`
          )
        }
        ...
        if (await page.isVisible(SEL.MSG_ERRO_LOGIN)) {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-erro`,
            `Prefeitura Vila Velha: falha de autenticação — ${msg?.trim() ?? 'erro desconhecido'}`
          )
        }
        ...
        if (page.url().toLowerCase().includes('login')) {
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-redirect`,
            `Prefeitura Vila Velha: ainda na tela de login após submit`
          )
        }
      ...
      this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-xml-${doc.numero}`,
            `Prefeitura Vila Velha: falha download XML nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
      ...
      this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-pdf-${doc.numero}`,
            `Prefeitura Vila Velha: falha download PDF nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
  ...
  private async garantirSessao(page: Page, cnpj: string, ctx: string): Promise<void> {
    if (!this.session)
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-sem-sessao-${ctx}`,
        `Prefeitura Vila Velha: sessão não inicializada — chame authenticate() primeiro`
      )
    if (this.session!.expiresAt < nowBR())
      return this.captureAndThrow(
        ...
        `Prefeitura Vila Velha: sessão expirada — reautentique`
      )
  }
  ...
  private async preencherFiltro(
    ...
    try {
      ...
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-filtro-${ctx}`,
        `Prefeitura Vila Velha: formulário de consulta não encontrado (${ctx})`
      )
    }
