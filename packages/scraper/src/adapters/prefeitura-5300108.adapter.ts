/**
 * Adapter — Prefeitura de Brasília (IBGE 5300108)
 * Portal: NFS-e Brasília — https://nfse.df.gov.br
 */
...
const BASE_URL = 'https://nfse.df.gov.br'
const TAG = 'prefeitura_df'
...
export class Prefeitura5300108Adapter extends BasePLaywrightAdapter implements PrefeituraAdapter {
  tipo = 'NFSE_EMITIDA'
  fonte = 'PREFEITURA_DF'
  municipio = 'Brasília'
  ibge = '5300108'
      ...
      const session = await this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-form`,
            `Prefeitura Brasília: formulário de login não encontrado`
          )
        }
        ...
        if (await page.isVisible(SEL.MSG_ERRO_LOGIN)) {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-erro`,
            `Prefeitura Brasília: falha de autenticação — ${msg?.trim() ?? 'erro desconhecido'}`
          )
        }
        ...
        if (page.url().toLowerCase().includes('login')) {
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-redirect`,
            `Prefeitura Brasília: ainda na tela de login após submit`
          )
        }
      ...
      this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-xml-${doc.numero}`,
            `Prefeitura Brasília: falha download XML nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
      ...
      this.withPage(async (page) => {
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-pdf-${doc.numero}`,
            `Prefeitura Brasília: falha download PDF nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
  ...
  private async garantirSessao(page: Page, cnpj: string, ctx: string): Promise<void> {
    if (!this.session)
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-sem-sessao-${ctx}`,
        `Prefeitura Brasília: sessão não inicializada — chame authenticate() primeiro`
      )
    if (this.session!.expiresAt < nowBR())
      return this.captureAndThrow(
        ...
        `Prefeitura Brasília: sessão expirada — reautentique`
      )
  }
  ...
  private async preencherFiltro(
    ...
    try {
      ...
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-filtro-${ctx}`,
        `Prefeitura Brasília: formulário de consulta não encontrado (${ctx})`
      )
    }
