/**
 * Adapter — Prefeitura de Porto Alegre (IBGE 4314902)
 * Portal: NFS-e Porto Alegre — https://issqn.portoalegre.rs.gov.br
 */
...
const BASE_URL = 'https://issqn.portoalegre.rs.gov.br'
const TAG = 'prefeitura_porto_alegre'
...
export class Prefeitura4314902Adapter extends BasePLaywrightAdapter implements PrefeituraAdapter {
  tipo = 'NFSE_EMITIDA'
  fonte = 'PREFEITURA_PORTO_ALEGRE'
  ...
  async authenticate(cred: Credential): Promise<Session> {
    ...
    return this.withRetry(async () => {
      const session = await this.withPage(async (page) => {
        await page.goto(`${BASE_URL}/login.jsf`, { waitUntil: 'networkidle', timeout: 60_000 })
        ...
        try {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-form`,
            `Prefeitura Porto Alegre: formulário de login não encontrado`
          )
        }
        ...
        if (await page.isVisible(SEL.MSG_ERRO_LOGIN)) {
          ...
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-erro`,
            `Prefeitura Porto Alegre: falha de autenticação — ${msg?.trim() ?? 'erro desconhecido'}`
          )
        }
        ...
        if (page.url().toLowerCase().includes('login')) {
          return this.captureAndThrow(
            ...
            credData.login,
            `${TAG}-login-redirect`,
            `Prefeitura Porto Alegre: ainda na tela de login após submit`
          )
        }
  ...
  async fetchEmitidas(cnpj: string, periodo: Periodo): Promise<DocumentoRaw[]> {
    return this.withRetry(() =>
      this.withPage(async (page) => {
        ...
        await page.goto(`${BASE_URL}/prestador/consultarNfse.jsf`, {
  ...
  async fetchTomadas(cnpj: string, periodo: Periodo): Promise<DocumentoRaw[]> {
    return this.withRetry(() =>
      this.withPage(async (page) => {
        ...
        await page.goto(`${BASE_URL}/tomador/consultarNfse.jsf`, {
          waitUntil: 'networkidle',
          timeout: 60_000,
        })
  ...
  async downloadXML(doc: DocumentoRaw): Promise<string> {
    ...
    return this.withRetry(() =>
      this.withPage(async (page) => {
        ...
        const url = `${BASE_URL}/prestador/downloadXml.jsf?numero=${encodeURIComponent(doc.numero)}&cnpj=${doc.cnpjEmitente}`
        try {
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-xml-${doc.numero}`,
            `Prefeitura Porto Alegre: falha download XML nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
  ...
  async downloadPDF(doc: DocumentoRaw): Promise<Buffer> {
    ...
    return this.withRetry(() =>
      this.withPage(async (page) => {
        ...
        const url = `${BASE_URL}/prestador/imprimirDanfse.jsf?numero=${encodeURIComponent(doc.numero)}&cnpj=${doc.cnpjEmitente}`
        try {
          const [download] = await Promise.all([
            page.waitForEvent('download', { timeout: 60_000 }),
            page.goto(url, { waitUntil: 'commit', timeout: 30_000 }),
          ...
          return this.captureAndThrow(
            ...
            doc.cnpjEmitente,
            `${TAG}-pdf-${doc.numero}`,
            `Prefeitura Porto Alegre: falha download PDF nota ${doc.numero} — ${(err as Error).message}`
          )
        }
      })
    )
  ...
  override async healthCheck(): Promise<boolean> {
    try {
      return await this.withPage(async (page) => {
        const res = await page.goto(`${BASE_URL}/login.jsf`, {
  ...
  private async garantirSessao(page: Page, cnpj: string, ctx: string): Promise<void> {
    if (!this.session)
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-sem-sessao-${ctx}`,
        `Prefeitura Porto Alegre: sessão não inicializada — chame authenticate() primeiro`
      )
    if (this.session!.expiresAt < nowBR())
      return this.captureAndThrow(
        ...
        `Prefeitura Porto Alegre: sessão expirada — reautentique`
      )
  }
  ...
  private async preencherFiltro(
    ...
    try {
      ...
      return this.captureAndThrow(
        ...
        cnpj,
        `${TAG}-filtro-${ctx}`,
        `Prefeitura Porto Alegre: formulário de consulta não encontrado (${ctx})`
      )
    }
