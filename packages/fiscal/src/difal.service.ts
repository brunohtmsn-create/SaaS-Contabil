...
export class DifalService {
  ...
  async calcular(
    ...
    for (const doc of docs) {
      ...
      const difal = baseCalculo
        .times(aliquotaInterna.minus(aliquotaInterestadual))
        .div(100)
        .toDecimalPlaces(2)
    ...
    await this.audit.registrar({
      ...
      estadoNovo: {
        competencia,
        totalDocs: resultados.length,
        totalDifal: resultados.reduce((s, r) => s.plus(r.valorTotal), new Decimal(0)).toString(),
      },
      responsavel: 'sistema',
  ...
  private getAliquotaInterestadual(ufOrigem: string, ufDestino: string): Decimal {
    ...
    return regioesSulSudesteCO.includes(ufDestino)
